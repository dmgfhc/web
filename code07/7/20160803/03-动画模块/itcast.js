...
(function(window){
    ...
    itcast.fn=itcast.prototype={
        ...
        init:function(selector,context){
            ...
            //判断参数是不是一个字符串，如果是字符串：a、选择器、b、html(<开头 >结尾)
            //$("<input type=='text'/>")
            if(itcast.isString(selector)){
                ...
                if(selector.charAt(0)==="<" && selector.charAt(selector.length-1)===">" && selector.length>=3){
                    ...
                    //遍历div的子节点
                    push.apply(this,div.childNodes);//--->{ 0:input,length:1 }
                ...
                // this[this.length]=selector;//this[0]=document.body
                // this.length++;//{0:body,length:1}
    ...
    itcast.fn.extend({
        appendTo: function (selector) {
            var $parents = itcast(selector);

            //this:-->{0:input,1:input,length:2}
            return this.each(function () {
                var child = this;//子元素
                //再遍历父元素
                $parents.each(function (parentIndex) {
                    var parent = this;//父元素
                    //在父元素中添加子元素——>appendChild
...
//                var $children=itcast(selector);
//                var $parents=this;
//
//                $children.appendTo($parents);
//                return $parents;

            //this：init类型的对象-->itcast(this)-->(this instanceof itcast)-->return this
            itcast(selector).appendTo(this);//
            // -->appendTo方法的参数，相当于给itcast入口函数传递一个init类型的对象
            return this;
        },
        //移除自身及其子元素——>移除自身(移除DOM元素——>从页面中删除，必须使用DOM操作的方式)
        //$("")
        remove: function () {
            //原生JS移除子元素：父元素.removeChild(子元素)

            //遍历
            return this.each(function () {
                var child = this;//即将删除的子元素   child=d3
                //获取child的父元素：parentNode
                var parent = child.parentNode;//parent=body
                //删除元素
